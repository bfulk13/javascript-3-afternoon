...
//Code Here
let postTaxPrices = prices.map( (elem,i,arr) => elem *= 1.07 )// = prices.map(/* Provide Your Callback Here );
...
//  })//let myStrongest = monstersInYourPocket.filter(monstersInYourPocket[CP] => {} )// = monstersInYourPocket.filter(/* Provide Your Callback Here */)
...
// Code here
 let bobsTotal = orders.filter( person => person.owner === `Bob` ).reduce( (acc, order) => acc + order.price );
